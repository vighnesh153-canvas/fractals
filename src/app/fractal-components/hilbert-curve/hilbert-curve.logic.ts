...
export class HilbertCurveLogic {
  canvas: Canvas;
  ...
  constructor(canvasElement: HTMLCanvasElement, order: number, stopper: () => void) {
      ...
      switch (index & 3) {
        case 0:
          [firstOrderPoint.x, firstOrderPoint.y] =
            [firstOrderPoint.y, firstOrderPoint.x];
          break;
        case 1:
        ...
        case 3:
          [firstOrderPoint.x, firstOrderPoint.y] =
            [length - 1 - firstOrderPoint.y, length - 1 - firstOrderPoint.x];
          firstOrderPoint.x += length;
      }
    }
  ...
  drawPoints() {
    ...
    for (let i = 1; i < min(this.framesCounter, this.points.length); i++) {
      ...
      this.canvas.drawLine(
        prev.x, prev.y, current.x, current.y, this.lineThickness, 'white'
      );
      prev = current;
    }
  }
